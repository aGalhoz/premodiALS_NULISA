...
plot_detectability_disease_group <- function(df, fluid, proteins_per_page = 30) {
  ...
  df_f <- df_f %>%
    group_by(Target) %>%
    mutate(overall_mean = mean(TargetDetectability_value, na.rm = TRUE)) %>%
    ungroup() %>%
    mutate(Target = factor(Target, 
                           levels = unique(Target[order(overall_mean, decreasing = TRUE)])))
  
  # Get unique targets and split them into chunks
  targets <- levels(df_f$Target)
  target_chunks <- split(targets, ceiling(seq_along(targets) / proteins_per_page))
  
  pdf_path <- paste0("plots/detectability/Detectability_DiseaseGroup_", fluid, ".pdf")
  Cairo::CairoPDF(pdf_path, width = 16, height = 7)
  
  for (i in seq_along(target_chunks)) {
    chunk_df <- df_f %>% filter(Target %in% target_chunks[[i]])
    ...
    p <- ggplot(chunk_df, 
      ...
      theme_bw(base_size = 14) +
      labs(
        title = paste("Target Detectability by Group -", fluid),
        x = "Target",
        y = "Detectability (NPQ - LOD)",
        fill = "Group"
      ) +
...
plot_detectability_dots <- function(df, fluid) {
  
  # check which targets are below detectability
  problem_targets <- df %>%
    filter(SampleMatrixType == fluid, pct_detected < 50) %>%
    pull(Target) %>% unique()
  
  df_f <- df %>% 
    filter(SampleMatrixType == fluid, Target %in% problem_targets)
  ...
  ggplot(df_f, aes(x = pct_detected, y = Target, color = type)) +
    geom_vline(xintercept = 50, color = "red", linetype = "dashed") +
    geom_point(alpha = 0.7, size = 2) +
    scale_color_manual(values = disease_colors) +
    theme_bw(base_size = 13) +
    theme(axis.text = element_text(size = 11)) +
    labs(title = paste("Detectability percentage per disease group in", fluid),
         x = "% Detected", y = "Protein")
}
...
samples_ID_type <- sample_ID_info %>%
  ...
  left_join(bind_rows(ALS_ID, CTR_ID, PGMC_ID), by = "ParticipantCode") %>%
...
effects_per_fluid <- df_f %>%
  ...
  group_modify(~ {
    model <- lm(NPQ ~ type + age + sex + center, data = .x)
    tidy(Anova(model,type = 3)) %>%
      mutate(Fluid = .y$SampleMatrixType)
  }) %>%
  ungroup()

effects_per_fluid <- effects_per_fluid %>%
...
var_fluid_df_summary = var_fluid_df$summary %>%
  left_join(effects_per_fluid %>%
              filter(term %in% c("type","age","sex","center")) %>%
              rename(variable = term) %>%
              select(Fluid,variable,p.value,significance))

writexl::write_xlsx(var_fluid_df_summary,"results/variance_by_covariate_fluid.xlsx")
...
for (fluid in fluids) {
  ...
  Cairo::CairoPDF(paste0("plots/detectability/TargetDetectability_", fluid, ".pdf"),
                  width = 28, height = 6)
  print(p1)
  dev.off()
  
  Cairo::CairoPDF(paste0("plots/detectability/TargetDetectability_byPlate_", fluid, ".pdf"),
                  width = 28, height = 8)
  print(p2)
  dev.off()
}

# target detectability by disease group
td_group <- protein_data %>%
  filter(SampleType == "Sample") %>%
  left_join(samples_ID_type %>% rename(SampleName = `Sample ID`), by = "SampleName") %>%
  select(PlateID, SampleMatrixType, type, Target, NPQ) %>% 
  filter(!is.na(type)) %>%
  left_join(target_detectability %>% rename(Target = TargetName), by = c("Target","PlateID")) %>%
  mutate(
    TargetDetectability_value = NPQ - TargetLOD_NPQ,
    TargetDetectability = as.numeric(TargetDetectability) * 100
  ) %>%
  mutate(TargetDetectability_value = ifelse(Target %in% c("KNG1", "CRP"),
                                            abs(TargetDetectability_value),
                                            TargetDetectability_value))

for (fluid in fluids) {
  
  plot_detectability_disease_group(td_group,fluid)
  
}

# check percentage of detectability per disease group
detectability_group_summary <- td_group %>%
  group_by(SampleMatrixType, type, Target) %>%
  summarise(
    pct_detected = mean(TargetDetectability_value > 0, na.rm = TRUE) * 100, # % of samples where NPQ > LOD
    .groups = 'drop'
  ) %>%
  mutate(low_detectability = pct_detected < 50)

for (fluid in fluids) {
 
  p = plot_detectability_dots(detectability_group_summary,fluid)
  
  Cairo::CairoPDF(paste0("plots/detectability/Lowdetected_byDiseaseGroup_", fluid, ".pdf"),
                  width = 7, height = 8)
  print(p)
  dev.off()
...
protein_data_with_lod <- protein_data %>%
  left_join(target_detectability_extra %>%
              ...
              distinct()) %>%
  mutate(below_lod = ifelse(Target %in% c("KNG1","CRP"),
                            NPQ > ProjectLOD,
                            NPQ < ProjectLOD))
...
detectability_summary = detectability_summary %>%
  mutate(n_samples = ifelse(SampleMatrixType == "CSF" & Target %in% c("CRP","KNG1"),
                            NA,n_samples),
         n_below_lod = ifelse(SampleMatrixType == "CSF" & Target %in% c("CRP","KNG1"),
                              NA,n_below_lod),
         frac_below_lod =  ifelse(SampleMatrixType == "CSF" & Target %in% c("CRP","KNG1"),
                                  NA,frac_below_lod),
         percent_below_lod = ifelse(SampleMatrixType == "CSF" & Target %in% c("CRP","KNG1"),
                                    NA,percent_below_lod),
         detectability = ifelse(SampleMatrixType == "CSF" & Target %in% c("CRP","KNG1"),
                                NA,detectability))
