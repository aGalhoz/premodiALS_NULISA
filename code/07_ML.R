...
# ================================================
# Check the best protein signature based on CV
# ================================================
...
## -> Lasso + Serum
proteins_serum_ALS_CTR = c("CLEC4A","SELE","TNFRSF13C","IL18R1","CD276","CD200",
                           "BST2","IFNL1","TNFSF14","HAVCR1")
...
## -> Lasso + Plasma
proteins_plasma_ALS_CTR = c("IL1RL1","TSLP","SELE","CHI3L1","SCG2","CD200",
                            "TNFRSF13C","TREM2","IL16","CXCL10")
lasso_optimal_protein_signature_plasma = find_optimal_signature(results_ALL = results_ALL,
                                                               ranked_proteins = proteins_plasma_ALS_CTR,
                                                               fluid = "PLASMA",
                                                               output_prefix = "plots/ML/Lasso/performance_")

# Elastic Net + Serum
proteins_serum_ALS_CTR = c("CLEC4A","SELE","CD276","TNFRSF13C","BST2","CD200","IL18R1",
                           "TREM2","IL1RL1","CTLA4")
...
# Elastic Net + Plasma
proteins_plasma_ALS_CTR = c("IL1RL1","TSLP","SELE","SCG2","CXCL10","CD200","CHI3L1",
                            "TREM2","TNFRSF13C","IL16")
...
pca_results_serum_adj$scores <- pca_results_serum_adj$scores %>%
  mutate(ParticipantCode = ifelse(ParticipantCode %in% participant_code_label,
                                  ParticipantCode,NA))
...
## -> Lasso + Plasma
proteins_plasma_ALS_CTR = lasso_optimal_protein_signature_plasma$optimal_proteins

lasso_PGMC_plasma_signature = run_ALS_signature_workflow(results_ALL,
                                                        proteins_plasma_ALS_CTR,
                                                        fluid = "PLASMA",
                                                        model_type = "lasso",
                                                        output_prefix = "plots/ML/Lasso/")

participant_code_label = lasso_PGMC_plasma_signature$results %>%
  left_join(protein_data_IDs %>% select(SampleName,ParticipantCode,type)) %>%
  distinct() %>%
  filter(type == "PGMC" & ALS_risk_score > 0) %>%
  pull(ParticipantCode)
...
# ================================================================================
# Unsupervised visualisation (PCA) of PGMC, ALS, CTR based on 10-protein signature
protein_data_PCA_plasma = protein_data_clean %>%
  filter(Target %in% lasso_optimal_protein_signature_plasma$optimal_proteins) %>%
  filter(type %in% c("PGMC","ALS","CTR"))
...
plots_subtype_adj <- plot_pca(pca_results_plasma_adj,paste0("plasma based on ",
                                                            length(lasso_optimal_protein_signature_plasma$optimal_proteins) ,
                                                            " protein signature"))

pdf("plots/ML/Lasso/PCA_plasma_protein_signature.pdf", width = 8, height = 6.5) 
plots_subtype_adj
dev.off()
...
pdf("plots/ML/Lasso/PCA_plasma_protein_signature_label.pdf", width = 8, height = 6.5) 
plots_subtype_adj_label
dev.off()
...
run_heatmap_signature(results_ALL,
                      "PLASMA",
                      lasso_optimal_protein_signature_plasma$optimal_proteins,
                      lasso_PGMC_plasma_signature$results,
                      group_colors = group_colors,
                      output_prefix = "plots/ML/Lasso/heatmap_plasma",
                      highlight_ids = participant_code_label)
...
# ================================================================================
# Unsupervised visualisation (PCA) of PGMC, ALS, CTR based on 7-protein signature
protein_data_PCA_serum = protein_data_clean %>%
...
# ================================================================================
# Unsupervised visualisation (heatmap) of PGMC, ALS, CTR based on 7-protein signature
