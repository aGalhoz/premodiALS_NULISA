...
## signed p-values 
compute_signed <- function(df, logfc_col, comp_name){
  
  df %>%
    mutate(
      signed = sign(.data[[logfc_col]]) * log10_padj
    ) %>%
    select(Target, UniProtID, signed, DE) %>%
    rename(
      !!paste0("signed_", comp_name) := signed,
...
signed_plot <- function(data, xvar, yvar, title, colors){
  
  ggplot(data, aes(.data[[xvar]], .data[[yvar]])) +
    
    geom_point(aes(colour = DE_new), size = 2.25, alpha = 0.8) +
    
    geom_vline(
      xintercept = c(log10(0.1), -log10(0.1)),
      linetype = "dashed",
      colour = "grey",
      alpha = 0.6
    ) +
    ...
    geom_hline(
      yintercept = c(log10(0.1), -log10(0.1)),
      linetype = "dashed",
      colour = "grey",
      alpha = 0.6
    ) +
...
datasets <- list(
  ...
  plasma = list(
    raw = list(
      ALS_CTR = DE_plasma_ALS_CTR,
      ALS_PGMC = DE_plasma_ALS_PGMC,
      mimic_CTR = DE_plasma_mimic_CTR,
      C9orf72_CTR = DE_plasma_C9orf72_CTR,
      SOD1_CTR = DE_plasma_SOD1_CTR),
    adjusted = list(
      ALS_CTR = DE_plasma_ALS_CTR_adj,
      ALS_PGMC = DE_plasma_ALS_PGMC_adj,
      mimic_CTR = DE_plasma_mimic_CTR_adj,
      C9orf72_CTR = DE_plasma_C9orf72_CTR_adj,
      SOD1_CTR = DE_plasma_SOD1_CTR_adj)),
  # CSF
  csf = list(
    raw = list(
      ALS_CTR = DE_CSF_ALS_CTR,
      ALS_PGMC = DE_CSF_ALS_PGMC,
      mimic_CTR = DE_CSF_mimic_CTR,
      C9orf72_CTR = DE_CSF_C9orf72_CTR,
      SOD1_CTR = DE_CSF_SOD1_CTR),
    adjusted = list(
      ALS_CTR = DE_CSF_ALS_CTR_adj,
      ALS_PGMC = DE_CSF_ALS_PGMC_adj,
      mimic_CTR = DE_CSF_mimic_CTR_adj,
      C9orf72_CTR = DE_CSF_C9orf72_CTR_adj,
      SOD1_CTR = DE_CSF_SOD1_CTR_adj)),
  # SERUM
  serum = list(
    raw = list(
      ALS_CTR = DE_SERUM_ALS_CTR,
      ALS_PGMC = DE_SERUM_ALS_PGMC,
      mimic_CTR = DE_SERUM_mimic_CTR,
      C9orf72_CTR = DE_SERUM_C9orf72_CTR,
      SOD1_CTR = DE_SERUM_SOD1_CTR),
    adjusted = list(
      ALS_CTR = DE_SERUM_ALS_CTR_adj,
      ALS_PGMC = DE_SERUM_ALS_PGMC_adj,
      mimic_CTR = DE_SERUM_mimic_CTR_adj,
      C9orf72_CTR = DE_SERUM_C9orf72_CTR_adj,
      SOD1_CTR = DE_SERUM_SOD1_CTR_adj)))
...
for(tissue in names(datasets)){
  ...
  for(version in names(datasets[[tissue]])){
    ...
    version_label <- ifelse(version == "raw", "", "_adjusted")
